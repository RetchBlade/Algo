...
class Liste { 
    ...
    int hashFunktion (String x,int b) { 
    	 int hash = 0; // Initialisierung eines Hash-Werts
    	    for (int i = 0; i < x.length(); i++) {
    	        hash += x.charAt(i); // Summiere die ASCII-Werte der Zeichen
    	    }
    	    return hash % b; // Reduzierung des Hashwerts auf gültigen Bucket-Index
    } 
	...
	public static void main(String args[]) {
		...
		while (true) {
			...
			if (menueeingabe.equals("2")) {
				...
				if (!existsInHashtable) {
					ausgabe = "Eingabe nicht gefunden in bucket ";
				}
				JOptionPane.showMessageDialog(null, ausgabe);
				//...
			...
			if (menueeingabe.equals("3")) {
				...
				for (int j = 1; j <= hashTabelle[h].laenge(); j++) {
					if (hashTabelle[h].inhalt(j).equals(dialogEingabe)) {
						ausgabe = "Element in Korb " + h+ " wurde geloescht";
						existsInHashtable = true;
